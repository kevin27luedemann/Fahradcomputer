...
class uhr:public monitor
{
	...
	void draw()
	{
        ...
        if(posx==3){
           ...
           EEPWriteByte(EEWECKMINUTEN,rtc->WMinuten); 
           posx = 0;
        }
        if(posx==0){
...
class einstellungen: public monitor
{
	...
	void draw(){
		...
		if (posx==0)
		{
			...
			buffersize=sprintf(buffer,"Logging SD: %u", (bool)(statusreg&(1<<mounttingstat))+(bool)(statusreg&(1<<loggingstat)));
		...
		else if (posy==2 && posx==1)
		{
			...
			else if ((statusreg&(1<<loggingstat)) && (statusreg&(1<<mounttingstat))){
				f_sync(&logger);
				f_close(&logger);
				statusreg &= ~(1<<loggingstat);
				f_mount(0,"",0);
